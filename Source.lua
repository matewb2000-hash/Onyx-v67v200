...
-- Executor: Synapse X / KRNL / Fluxus / Real
...
local ARENA_MIN = Vector3.new(-150, -50, -150)
local ARENA_MAX = Vector3.new(350, 100, 350)
...
makeSlider(farmTab, "Farm Delay", "FarmDelay", 0, 1, 4)
...
makeButton(playerTab, "Apply Speed and Jump", 5, function()
    local char = lp.Character
    if not char then return end
...
RunService.Heartbeat:Connect(function()
    ...
    if closest then
        hrp.CFrame = CFrame.new(
            closest.Position + Vector3.new(2, 0, 0),
            closest.Position
        )
        ...
        if now - lastAttack >= ATTACK_COOLDOWN then
            ...
            for _, v in ipairs(remotes:GetDescendants()) do
                if v:IsA("RemoteEvent") then
                    local n = v.Name:lower()
                    if n:find("attack") or n:find("hit") or n:find("stab") or n:find("dmg") then
